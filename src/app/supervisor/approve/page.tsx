...
import { ClipboardCheck, Check, X, Clock, CheckCircle, AlertTriangle, ChevronDown, ChevronUp } from "lucide-react";
...
export default function SupervisorApproval() {
  ...
  const approve = async (logId: string) => {
    const res = await fetch("/api/daily-production", {
      ...
      body: JSON.stringify({ action: "supervisor_approve", logId }),
    });
    if (res.ok) {
      setSuccess("Approved! Manager notified.");
      fetchLogs();
  ...
  const approvedLogs = logs.filter((l) => l.status === "SUPERVISOR_APPROVED" || l.status === "MANAGER_APPROVED");
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <main className="ml-0 md:ml-64 p-4 md:p-8">
        <div className="mb-6">
          ...
          <p className="text-slate-400 text-sm mt-1">Review and approve daily production logs</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            {approvedLogs.length > 0 && (
              <div>
                <h2 className="text-slate-400 font-bold text-sm mb-3">APPROVED ({approvedLogs.length})</h2>
                ...
                {showReject === log.id ? (
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => approve(log.id)}
                      className="flex-1 py-3.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold rounded-xl shadow-lg active:scale-[0.97] transition flex items-center justify-center gap-2">
