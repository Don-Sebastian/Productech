...
export default function SupervisorApprovalPage() {
  ...
  const sessions: HotPressSession[] = apiData?.pendingSessions || [];

  const approve = async (sessionId: string) => {
    // Direct approve without confirm dialog
    const res = await fetch("/api/hotpress", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "supervisorApprove", sessionId }),
    });
    if (res.ok) {
      await fetchData();
    } else {
      ...
      {isRejecting ? (
        ...
        <div className="p-4 flex gap-2">
          <button onClick={onApprove}
            ...
            <CheckCircle2 size={22} /> Approve & Forward
          </button>
